...
class SimulationView:
    ...
    def index(request):
        ...
        simulators = [
            sp
            for sp in SimulatorProfile.objects.all()
            if sp.name == (request.GET.get("simulator_filter") or sp.name)
        ]
        hardware_profiles = [
            hp
            for hp in HardwareProfile.objects.all()
            if hp.name == (request.GET.get("hardware_filter") or hp.name)
        ]
        ...
        envs = [
            SimulationEnv(hp, sp)
            for hp, sp in itertools.product(hardware_profiles, simulators)
            if hp.uuid in hps and sp.name in sps
        ]
        ...
        values = {
            "qubits": set(SimulationRun.objects.values_list("qubits", flat=True))
            or {0},
        ...
        for name in ["qubits", "depth", "shots"]:
            max_v = min(
                int(request.GET.get(name + "_max") or max(values[name])),
                max(values[name]),
            )
            min_v = max(
                int(request.GET.get(name + "_min") or min(values[name])),
                min(values[name]),
            )

            range_v = (
                set(v for v in values[name] if v in range(min_v, max_v + 1))
                if selected_range == name
                else {max_v}
            )
        ...
        for env in envs:
            ...
            runs = SimulationRun.objects.filter(
                simulator=env.simulator.name, hardware=env.hardware.uuid, **conf_filter
            )

            data.append({"label": name, "data": [run.duration_avg for run in runs]})
